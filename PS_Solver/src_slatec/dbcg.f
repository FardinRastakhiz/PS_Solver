...
C***ROUTINES CALLED  D1MACH, DAXPY, DCOPY_SLATEC, DDOT_SLATEC, ISDBCG
...
C     .. External Functions ..
      DOUBLE PRECISION D1MACH, DDOT_SLATEC
      INTEGER ISDBCG
      EXTERNAL D1MACH, DDOT_SLATEC, ISDBCG
C     .. External Subroutines ..
      EXTERNAL DAXPY, DCOPY_SLATEC
...
C         Calculate coefficient BK and direction vectors P and PP.
         BKNUM = DDOT_SLATEC(N, Z, 1, RR, 1)
         IF( ABS(BKNUM).LE.FUZZ ) THEN
            IERR = 6
            RETURN
         ENDIF
         IF(ITER .EQ. 1) THEN
            CALL DCOPY_SLATEC(N, Z, 1, P, 1)
            CALL DCOPY_SLATEC(N, ZZ, 1, PP, 1)
         ELSE
            BK = BKNUM/BKDEN
            DO 20 I = 1, N
               P(I) = Z(I) + BK*P(I)
